...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase
    .from("scheduled_jobs")
    .select(
      "id, day_of_week, sort_order, job_definition_id, job_definitions ( id, name, sort_order, job_requirements ( position, description ) ), job_punts ( scheduled_job_id, user_id, users!job_punts_user_id_fkey ( id, username ) ), job_submissions ( id, scheduled_job_id, submitted_at, user_id, review_status, verified_by_admin, users!job_submissions_user_id_fkey ( id, username ) )"
    )
    .eq("week_id", weekId)
